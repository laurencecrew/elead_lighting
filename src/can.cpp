...
bool CANInit(BITRATE bitrate, int remap)
{
  ...
  #ifdef DEBUG
    DebugSerial.print("can1=");
    DebugSerial.println(can1);
  #endif
  if (can1) {
    #ifdef DEBUG
      DebugSerial.println("CAN1 initialize ok");
    #endif
  } else {
    #ifdef DEBUG
      DebugSerial.println("CAN1 initialize fail!!");
    #endif
    return false;
  }
  return true; 
...
void CANSend(CAN_msg_t* CAN_tx_msg)
{
  ...
  if (CAN1->sTxMailBox[0].TIR & 0x1UL) {
      ...
      DebugSerial.println(CAN1->ESR);
      DebugSerial.println(CAN1->MSR);
      DebugSerial.println(CAN1->TSR);
    #endif
  }
}
